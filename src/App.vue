...
<template>
  ...
  <main v-else class="app-shell">
    ...
    <section v-else class="page history-page"><div class="history-intro"><p>Твой дневник помогает замечать ситуации, в которых поддержка особенно нужна.</p><div class="history-summary"><span><b>{{ store.data.cravings.length }}</b> тяг</span><span><b>{{ store.data.slips.length }}</b> эпизодов</span></div></div><CravingHeatmap v-if="store.data.cravings.length" :logs="store.data.cravings"/><div class="history-filter" role="tablist"><button :class="{ active: historyFilter === 'all' }" @click="historyFilter = 'all'; impact()">Все</button><button :class="{ active: historyFilter === 'craving' }" @click="historyFilter = 'craving'; impact()">Тяги</button><button :class="{ active: historyFilter === 'slip' }" @click="historyFilter = 'slip'; impact()">Эпизоды</button></div><div v-if="filteredHistory.length" class="group-list history-list"><article v-for="event in filteredHistory" :key="event.id" class="history-row" :class="event.type"><div class="history-icon"><BriefcaseBusiness v-if="event.type === 'craving' && event.context?.location === 'work'"/><Coffee v-else-if="event.type === 'craving' && event.context?.emotion === 'coffee_alcohol'"/><Wind v-else-if="event.type === 'craving'"/><AlertTriangle v-else/></div><div class="history-copy"><div class="history-line"><b>{{ event.type === 'craving' ? 'Тяга · ' : 'Эпизод · ' }}{{ event.title }}</b><time>{{ new Date(event.timestamp).toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' }) }}</time></div><p>{{ event.subtitle }} · {{ new Date(event.timestamp).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }) }}</p><small v-if="event.note">{{ event.note }}</small></div></article></div><div v-else class="empty-state"><Wind :size="32"/><p>В этой категории пока нет записей.</p></div></section>
