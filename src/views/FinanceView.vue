...
<template>
  <section class="screen finance-screen">
    ...
    <div v-else-if="section === 'debts'" class="module-content">
      ...
      <div class="card debt-list"><div v-if="!filteredDebts.length" class="empty"><Landmark :size="28" /><span>{{ debtDirection === 'owed_to_me' ? 'Вам пока никто не должен' : 'Долгов нет' }}</span></div><div v-for="debt in filteredDebts" :key="debt.id" class="row debt-row"><span><b>{{ debt.title }}</b><small>{{ debt.dueDate ? `Срок: ${new Date(debt.dueDate).toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' })}` : 'Срок не указан' }}</small></span><b>{{ money(debt.amount) }} {{ store.state.finance.currency }}</b><button aria-label="Удалить долг" @click="store.removeDebt(debt.id); haptic('light')"><Trash2 :size="17" /></button></div></div>
    </div>

    <div v-else class="module-content"><section class="burn-hero"><Repeat2 :size="23" /><div><span>Фиксированная стоимость жизни</span><b>{{ money(dailyBurn) }} {{ store.state.finance.currency }} <small>/ день</small></b><small>{{ money(monthlyBurn) }} {{ store.state.finance.currency }} / месяц</small></div></section><section v-if="upcomingSubscriptions.length" class="reminder"><BellRing :size="19" /><p><b>{{ upcomingSubscriptions[0].title }}</b> {{ billingText(upcomingSubscriptions[0]) }}. Ты ещё пользуешься сервисом?</p></section><button class="add-inline subscription-add" @click="subscriptionSheetOpen = true; haptic('light')"><Plus :size="18" /> Добавить подписку</button><h2 class="section-title">Все сервисы</h2><div class="card subscription-list"><div v-for="subscription in sortedSubscriptions" :key="subscription.id" class="row subscription-row"><button class="subscription-switch" :class="{ active: subscription.active }" :aria-label="subscription.active ? 'Поставить на паузу' : 'Возобновить подписку'" @click="subscription.active = !subscription.active; haptic('light')" /><span><b>{{ subscription.title }}</b><small>{{ subscriptionMeta(subscription) }}</small></span><b>{{ money(subscriptionDailyCost(subscription)) }} {{ store.state.finance.currency }}<small>/день</small></b><button aria-label="Удалить подписку" @click="store.removeSubscription(subscription.id); haptic('light')"><Trash2 :size="17" /></button></div></div></div>
  </section>
