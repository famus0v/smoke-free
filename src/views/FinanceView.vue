...
const futureDailyLimit = computed(() => {
  const available = Math.max(0, store.state.finance.monthlyBudget - store.monthExpenses)
  const afterToday = Math.max(0, available - store.safeToSpend)
  return afterToday / Math.max(1, daysLeft.value - 1)
})
const nextDays = computed(() => [1, 2, 3].map((offset) => {
  const date = new Date()
  date.setDate(date.getDate() + offset)
  return { label: offset === 1 ? 'Завтра' : date.toLocaleDateString('ru-RU', { weekday: 'short', day: 'numeric' }), limit: futureDailyLimit.value }
}))
const add = () => {
...
<template>
  <section class="screen finance-screen">
    ...
    <p class="screen-subtitle">Лимит пересчитывается после каждого расхода.</p>

    <section class="finance-hero budget-hero">
      <span class="finance-hero-icon"><WalletCards :size="25" /></span>
      <p>ТРАТЫ СЕГОДНЯ</p>
      <b>{{ money(spentToday) }} <small>/ {{ money(store.safeToSpend) }} {{ store.state.finance.currency }}</small></b>
      <span>сегодняшний лимит</span>
    </section>

    <section class="card future-limits">
      <div class="future-title"><span>Лимит на следующие дни</span><b>{{ money(futureDailyLimit) }} {{ store.state.finance.currency }} / день</b></div>
